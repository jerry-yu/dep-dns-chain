...
fn it_should_register_domain_successfully() {
    new_test_ext().execute_with(|| {
		...
		assert_ok!(Dns::register_domain(RuntimeOrigin::signed(account_id), domain.clone(), owner_info.clone(), domain_alias.clone()));
    });
}
...
fn it_should_not_register_domain_if_exists() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Dns::register_domain(RuntimeOrigin::signed(account_id), domain.clone(), owner_info.clone(), domain_alias.clone()));
		assert_noop!(
			Dns::register_domain(RuntimeOrigin::signed(account_id), domain.clone(), owner_info.clone(), domain_alias.clone()),
			Error::<Test>::Existed
		);
	});
}
...
fn it_should_transfer_ownership() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Dns::register_domain(RuntimeOrigin::signed(old_owner), domain.clone(), owner_info.clone(), domain_alias.clone()));

		assert_ok!(Dns::transfer_ownershit(RuntimeOrigin::signed(old_owner), domain.clone(), new_owner));
	});
}
